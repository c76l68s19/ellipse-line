...
Point Ellipse::compute_nearest_intersection(Point point_on_line) const {
    ...
    if (pol.get_x() == 0 && pol.get_y() == 0) {
        throw std::logic_error("identical points do not define a line");
    }
    // Compute intersection. Source: https://mathworld.wolfram.com/Ellipse-LineIntersection.html
    const unsigned numerator{a * b};
    const unsigned denominator2{a * a * pol.get_y() * pol.get_y() + b * b * pol.get_x() * pol.get_x()};
    ...
    const Point intersection(coefficient * pol.get_x(), coefficient * pol.get_y());
